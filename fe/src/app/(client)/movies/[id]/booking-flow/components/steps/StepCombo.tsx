...
import { useProducts } from '@/lib/api/get/products';
...
export function StepCombo({ cartItems, updateCartItem }: StepComboProps) {
  ...
  const renderProductImage = (product: Product) => {
    ...
    return <div className={`text-4xl w-full h-full flex items-center justify-center ${product.imageUrl ? 'hidden' : ''}`}>{icon}</div>;
  };
            ...
            return (
              ...
              >
                ...
                <div className="w-20 h-20 bg-white rounded-xl flex items-center justify-center shadow-sm border border-gray-100 flex-shrink-0 overflow-hidden relative">
                  ...
                  {isOutOfStock && (
                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                        <span className="text-white text-[10px] font-bold px-1 py-0.5 bg-red-500 rounded">HẾT HÀNG</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-text-primary font-semibold truncate text-base mb-0.5" title={product.name}>
                    {product.name}
                  </h3>
                ...
                {/* Counter Controls */}
                <div className={`flex items-center gap-1 bg-white rounded-lg p-1 border border-border shadow-sm ${isOutOfStock ? 'pointer-events-none opacity-50' : ''}`}>
                  <button
