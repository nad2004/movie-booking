...
export function StepSeatSelection({ selectedSeats, schedule, onSeatClick }: StepSeatSelectionProps) {
  ...
  const rows = useMemo(() => {
    ...
    return Object.keys(groups).sort().map(rowLabel => ({
      rowLabel,
  ...
  const getSeatStatus = (seat: SeatAvailability) => {
    ...
    return seat.seatType === 'VIP' ? 'vip' : 
           seat.seatType === 'Ghế đôi' ? 'couple' : 'standard';
  };
  ...
  return (
    <div>
      ...
      <div className="mb-10">
        ...
        <div className="text-center text-xs text-text-secondary uppercase tracking-widest font-semibold">Màn hình</div>
      </div>
        ...
        <div className="flex flex-col items-center min-w-max gap-3">
          ...
          {rows.map(({ rowLabel, seats }) => (
            <div key={rowLabel} className="flex items-center gap-2 sm:gap-4">
              {/* Tên hàng (A, B, C...) */}
              <span className="w-6 text-text-secondary text-center font-bold text-sm">{rowLabel}</span>
