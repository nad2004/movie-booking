...
export function ReviewTable({ reviews, isLoading, onApprove, onRejectClick, onDeleteClick }: ReviewTableProps) {
  if (isLoading) return <div className="text-center py-10">Đang tải...</div>;
  if (reviews.length === 0) return <div className="text-center py-10 text-gray-500">Không có đánh giá nào.</div>;

  const getStatusBadge = (status: string) => {
    switch (status) {
      case "Đã duyệt": return <Badge className="bg-green-100 text-green-700 hover:bg-green-100">Đã duyệt</Badge>;
      case "Bị từ chối": return <Badge className="bg-red-100 text-red-700 hover:bg-red-100">Từ chối</Badge>;
      default: return <Badge className="bg-yellow-100 text-yellow-700 hover:bg-yellow-100">Chờ duyệt</Badge>;
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {reviews.map((review) => (
        <div key={review._id} className="bg-white border border-gray-200 p-6 rounded-xl hover:shadow-md transition-all flex flex-col h-full">
          {/* Header: User Info */}
