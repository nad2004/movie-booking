...
export function TicketTable({ bookings, isLoading, onEditStatus, onDelete }: TicketTableProps) {
  ...
  if (bookings.length === 0) return <div className="text-center py-10 text-gray-500">Không có vé nào.</div>;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'Đã sử dụng': return 'bg-blue-100 text-blue-700';
          ...
          {bookings.map((ticket) => (
            <TableRow key={ticket._id} className="hover:bg-gray-200/50 text-gray-950">
              ...
              <TableCell>
                ...
                <div className="text-xs text-gray-500">{ticket.theaterName} - {ticket.roomName}</div>
              </TableCell>
              <TableCell>
