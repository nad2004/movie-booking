...
test('test', async ({ page }) => {
  ...
  await expect(page.locator('div').filter({ hasText: /^Email hoặc mật khẩu không đúng$/ })).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Login', exact: true }).click();
  await expect(page.locator('div').filter({ hasText: 'Đăng nhập thành côngChào mừng' }).nth(1)).toBeVisible();
});